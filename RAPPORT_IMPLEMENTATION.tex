...
\begin{center}
{\large\bfseries\colorbox{primary}{\textcolor{white}{\quad Catégories de tests logiciels \quad}}}\\[0.4cm]
{\large\bfseries\textcolor{danger}{Conception et gestion des cas de test}}\\[0.2cm]
{\large\bfseries\textcolor{secondary}{Format de cas de test (modèle 1)}}
\end{center}
...
% ---- F1 ----
\vspace{0.2cm}
{\bfseries\textcolor{danger}{Fonction 1 : Créer un compte — \texttt{POST /accounts}}}
\vspace{0.1cm}
...
\hline
CT-01 & Création valide avec tous les champs &
...
HTTP 201 — Compte créé avec \texttt{id} UUID et \texttt{account\_number} générés. \texttt{balance = 10000} & PASS \\
\hline
\rowcolor{rowalt}
CT-02 & Création sans email ni solde initial &
...
HTTP 422 — Erreur : champ obligatoire manquant & PASS \\
\hline
\rowcolor{rowalt}
...
\textbf{\textcolor{white}{Test Case ID}} & \textbf{\textcolor{white}{Test Case Description}} & \textbf{\textcolor{white}{Test Input (Test Case Value)}} & \textbf{\textcolor{white}{Expected Results}} & \textbf{\textcolor{white}{Test Case Status}} \\
\hline
CT-08 & Consulter un compte existant & \texttt{account\_id} = UUID valide récupéré après création & HTTP 200 — Détails du compte avec l'historique complet des transactions & PASS \\
\hline
\rowcolor{rowalt}
...
% ---- F4 ----
\vspace{0.2cm}
{\bfseries\textcolor{danger}{Fonction 4 : Effectuer un dépôt — \texttt{POST /accounts/\{account\_id\}/deposit}}}
\vspace{0.1cm}
...
\textbf{\textcolor{white}{Test Case ID}} & \textbf{\textcolor{white}{Test Case Description}} & \textbf{\textcolor{white}{Test Input (Test Case Value)}} & \textbf{\textcolor{white}{Expected Results}} & \textbf{\textcolor{white}{Test Case Status}} \\
\hline
CT-10 & Dépôt valide sans description & Compte \texttt{balance = 10000} \newline \texttt{amount: 5000} & HTTP 200 — \texttt{balance = 15000}, transaction \texttt{deposit} enregistrée & PASS \\
\hline
\rowcolor{rowalt}
CT-11 & Dépôt valide avec description & Compte existant \newline \texttt{amount: 3000} \newline \texttt{description: "Depot agence"} & HTTP 200 — Libellé visible dans l'historique & PASS \\
\hline
CT-12 & Montant égal à zéro & Compte existant \newline \texttt{amount: 0} & HTTP 422 — \textit{"Le montant doit être strictement positif."} & PASS \\
\hline
\rowcolor{rowalt}
CT-13 & Montant négatif & Compte existant \newline \texttt{amount: -1000} & HTTP 422 — \textit{"Le montant doit être strictement positif."} & PASS \\
\hline
CT-14 & Dépôt sur compte inexistant & \texttt{account\_id} invalide \newline \texttt{amount: 5000} & HTTP 404 — \textit{"Compte introuvable."} & PASS \\
\hline
\end{longtable}
...
\textbf{\textcolor{white}{Test Case ID}} & \textbf{\textcolor{white}{Test Case Description}} & \textbf{\textcolor{white}{Test Input (Test Case Value)}} & \textbf{\textcolor{white}{Expected Results}} & \textbf{\textcolor{white}{Test Case Status}} \\
\hline
CT-15 & Retrait valide, solde suffisant & Compte \texttt{balance = 10000} \newline \texttt{amount: 2000} \newline \texttt{description: "Retrait DAB"} & HTTP 200 — \texttt{balance = 8000}, transaction \texttt{withdraw} enregistrée & PASS \\
\hline
\rowcolor{rowalt}
CT-16 & Retrait égal au solde disponible & Compte \texttt{balance = 10000} \newline \texttt{amount: 10000} & HTTP 200 — \texttt{balance = 0}, compte vidé & PASS \\
\hline
...
\hline
\rowcolor{rowalt}
CT-18 & Montant égal à zéro & Compte existant \newline \texttt{amount: 0} & HTTP 422 — \textit{"Le montant doit être strictement positif."} & PASS \\
\hline
CT-19 & Montant négatif & Compte existant \newline \texttt{amount: -500} & HTTP 422 — \textit{"Le montant doit être strictement positif."} & PASS \\
\hline
\rowcolor{rowalt}
...
\textbf{\textcolor{white}{Test Case ID}} & \textbf{\textcolor{white}{Test Case Description}} & \textbf{\textcolor{white}{Test Input (Test Case Value)}} & \textbf{\textcolor{white}{Expected Results}} & \textbf{\textcolor{white}{Test Case Status}} \\
\hline
CT-24 & Virement valide entre deux comptes & Compte A \texttt{balance = 10000}, Compte B \texttt{balance = 5000} \newline \texttt{to\_account\_id} = ID de B \newline \texttt{amount: 3000} & HTTP 200 — A : \texttt{balance = 7000}, B : \texttt{balance = 8000}. Transactions \texttt{transfer\_out} (A) et \texttt{transfer\_in} (B) & PASS \\
\hline
\rowcolor{rowalt}
...
\hline
\rowcolor{rowalt}
CT-27 & Virement vers le même compte & \texttt{to\_account\_id} = même \texttt{account\_id} \newline \texttt{amount: 1000} & HTTP 400 — \textit{"Le compte source et le compte destinataire doivent être différents."} & PASS \\
\hline
...
\textbf{\textcolor{white}{Test Case ID}} & \textbf{\textcolor{white}{Test Case Description}} & \textbf{\textcolor{white}{Test Input (Test Case Value)}} & \textbf{\textcolor{white}{Expected Results}} & \textbf{\textcolor{white}{Test Case Status}} \\
\hline
CT-21 & Historique d'un compte sans opération & Compte créé avec \texttt{initial\_balance = 0} \newline \texttt{GET /accounts/\{id\}/transactions} & HTTP 200 — Corps : \texttt{[]} & PASS \\
\hline
\rowcolor{rowalt}
CT-22 & Historique avec plusieurs opérations & Compte avec dépôt, retrait et virement effectués & HTTP 200 — Liste chronologique de toutes les transactions avec type, montant et solde après & PASS \\
\hline
